...
import PeopleTable from "../../components/PeopleTable/PeopleTable.component";
...
const PeoplePage = () => {
  ...
  const { people, isLoading, error } = useContext(SwapiContext);
  ...
  useEffect(() => {
    if (people && !isLoading) {
      ...
      const newRows: any = [];
      people.results.forEach((person: Record<string, any>) => {
        newRows.push(
          createData(
            person.name,
            person.birth_year,
  ...
  const handlePaginate = async (page: string | null) => {
    ...
    try {
      ...
      const newRows: any = [];
      newData.data.results.forEach((person: Record<string, any>) => {
        newRows.push(
          createData(
            person.name,
            person.birth_year,
  ...
  return (
    <div>
      ...
      {isLoading || isFetching ? (
        ...
        rows && <PeopleTable tableData={rows} />
      )}
